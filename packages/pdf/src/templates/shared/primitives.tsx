...
type IconProps = ComponentProps<typeof PhosphorIcon> & {
	nodeKey?: string | undefined;
	/** Skip the item-icon slot's display (hideIcons / flex). Level decorations must stay visible and in-flow. */
	ignoreItemSlotDisplay?: boolean | undefined;
};

export const Icon = ({ style, size: sizeProp, nodeKey, ignoreItemSlotDisplay = false, ...props }: IconProps) => {
	const { style: iconStyle, size: templateSize, display, ...iconProps } = useTemplateIconSlot("icon");
	...
	if ((!ignoreItemSlotDisplay && display === "none") || !visible) return null;

	return (
		<PhosphorIcon
			{...iconProps}
			{...props}
			{...(!ignoreItemSlotDisplay && display !== undefined ? { display } : {})}
