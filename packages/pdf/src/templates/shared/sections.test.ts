...
const source = readFileSync(fileURLToPath(new URL("./sections.tsx", import.meta.url)), "utf8").replaceAll("\r\n", "\n");
...
describe("EducationSection", () => {
	it("keeps location and period on the trailing edge when area of study is empty", () => {
		expect(source).toContain('{hasArea ? <Text semanticField="area">{item.area}</Text> : <View />}');
	});
});
