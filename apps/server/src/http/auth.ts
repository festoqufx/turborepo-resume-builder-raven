...
import { withLocalSessionIfNeeded } from "./local-session";
...
export async function handleAuth(request: Request) {
	...
	const response = await auth.handler(finalRequest);
	return withLocalSessionIfNeeded(finalRequest, response);
}
...
export async function handleOAuth(request: Request) {
	...
	dashboardUrl.searchParams.set("callbackURL", `/api/auth/oauth?${oauthParams.toString()}`);

	return new Response(null, {
		status: 302,
		headers: { Location: `${dashboardUrl.pathname}${dashboardUrl.search}` },
	});
}
