...
describe("handleOAuth", () => {
	it("redirects unauthenticated users to the dashboard", async () => {
		...
		const dashboardUrl = new URL(location ?? "", "http://localhost:3000");
		const callbackUrl = new URL(dashboardUrl.searchParams.get("callbackURL") ?? "", "http://localhost:3000");

		expect(dashboardUrl.origin).toBe("http://localhost:3000");
