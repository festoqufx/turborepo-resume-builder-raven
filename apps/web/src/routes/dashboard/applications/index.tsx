...
import { DashboardQueryError } from "../-components/query-error";
...
function RouteComponent() {
	...
	useEffect(() => {
		...
		void navigate({ search: (prev: Search) => ({ ...prev, search: debouncedSearch }) });
	...
	}, [applications, searchDraft, tags, sort, archived]);
	...
	return (
		<div className="flex h-[calc(100dvh-2rem)] flex-col gap-4">
			...
			{isPending ? (
				...
				<DashboardQueryError onRetry={() => void refetch()} />
			) : isEmpty ? (
...
function ApplicationsSkeleton() {
	return (
		<div className="flex min-h-0 flex-1 flex-col gap-3" aria-hidden="true">
			<div className="flex items-center gap-2">
				<Skeleton className="h-9 max-w-72 flex-1" />
				<Skeleton className="h-9 w-24" />
				<Skeleton className="h-9 w-28" />
			</div>
			<div className="grid min-h-0 flex-1 grid-cols-1 gap-3 md:grid-cols-3">
				<Skeleton className="h-full min-h-48 rounded-md" />
				<Skeleton className="h-full min-h-48 rounded-md" />
				<Skeleton className="h-full min-h-48 rounded-md" />
			</div>
		</div>
